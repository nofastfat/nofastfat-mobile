package com.xy.view.ui.ctrls {
	...
	public class DecorateContainer extends AbsContainer {
		...
		public function DecorateContainer() {
			
			super();
			_addBtn = Tools.makeBlackButton("更多装饰");
		...
		private function updateShow() : void {
			...
			if (needLoad) {
				MulityLoad.getInstance().load(loads, loadOk, SourceType.DECORATE);
			} else {
		...
		override public function resize(height : int) : void {
			...
			if (_scrollUI != null) {
				_itemMaxHeight = height - (_addBtn.y + _addBtn.height) - _sizeUI.height - 10;
